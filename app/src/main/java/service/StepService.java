...
public class StepService extends Service implements SensorEventListener{
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
         ...
         * 总之有需要的可以和我交流
         */
        nfIntent = new Intent(this, PedometerActivity.class);
        builder.setContentIntent(PendingIntent.getActivity(this, 0, nfIntent, 0)) // 设置PendingIntent
                .setLargeIcon(BitmapFactory.decodeResource(this.getResources(), R.drawable.v14)) // 设置下拉列表中的图标(大图标)
                .setContentTitle("今日步数"+CURRENT_STEP+"步") // 设置下拉列表里的标题
                .setSmallIcon(R.drawable.v14) // 设置状态栏内的小图标
                .setContentText("加油，要记得勤加运动"); // 设置上下文内容
        // 获取构建好的Notification
        Notification stepNotification = builder.build();

        manager.notify(110,stepNotification);
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what){
                case Config.MSG_FROM_CLIENT:
                    ...
                    try {
                        //这里负责将当前的步数发送出去，可以在界面或者其他地方获取，这里是在Pedometer中获取来更新界面
    ...
    private void initTodayData() {
        ...
        //获取数据库
        dao = new StepDataDao(getApplicationContext());
        //获取当天数据，用于展示
        StepCounter counter = dao.getCurDataByDate(CURRENT_DATE);
        //为空则说明还没有当天数据，有则说明已经开始计步了
        if (counter == null) {
            CURRENT_STEP = 0;
        } else {
            CURRENT_STEP = Integer.parseInt(counter.getSteps());
        }
    }
    ...
    private void addCountStepListener() {
        ...
        if(countSensor != null){
            stepSensor = 0;
            sensorManager.registerListener((SensorEventListener) StepService.this,countSensor,SensorManager.SENSOR_DELAY_NORMAL);
        }else if(detectorSensor != null){
            stepSensor = 1;
            sensorManager.registerListener((SensorEventListener) StepService.this,detectorSensor,SensorManager.SENSOR_DELAY_NORMAL);
        }
    }
    ...
    private void saveStepData(){
        //查询数据库中的数据
        StepCounter counter = dao.getCurDataByDate(CURRENT_DATE);
        //为空则说明没有当日的数据，有则说明已经开始计步
        if(counter == null){
            //没有则新建一天数据
            counter = new StepCounter();
            counter.setCurDate(CURRENT_DATE);
            counter.setSteps(String.valueOf(CURRENT_STEP));
            dao.addNewData(counter);
        }else {
            //有则更新当前数据
            counter.setSteps(String.valueOf(CURRENT_STEP));
            dao.updateCurData(counter);
        }

        builder.setContentIntent(PendingIntent.getActivity(this,0,nfIntent,0))//设置PendingIntent
                .setLargeIcon(BitmapFactory.decodeResource(this.getResources(), R.drawable.v14))//设置下拉列表的大图标
                .setContentTitle("今日步数"+CURRENT_STEP+"步")//设置下拉列表标题
                .setSmallIcon(R.drawable.v14)//设置状态栏小图标
                .setContentText("加油，要记得勤加运动");//设置推送内容
